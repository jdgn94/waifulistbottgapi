...
const addExpUser = async (userInfoId: number, expType: expType, t: Transaction) => {
  ...
  if (expType.type == 'newWaifu' && userInfo) {
    const addExp = expType.imgFavorite ? await numberRandom(20, 15) : await numberRandom(15, 10);
...
const messageExp = async (exp: number, newLevel: number, oldLevel: number, newLimit: number) => {
