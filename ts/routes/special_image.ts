  ...
  try {
    ...
    const specialList = await Promise.all(
      await list.map(async (item: any) => {
        const waifusTemp = await waifus.filter((waifu: any) => waifu.id == item.id);
        const waifusTempName = await Promise.all(waifusTemp.map((waifu: any) => waifu.name));
...
router.post('/create', async (req: Request, res: Response) => {
  ...
  try {
    ...
    let insert: any = [];
    await waifuIds.forEach(async (id: number) => await insert.push({ special_image_id: special.id, waifu_id: id }));
...
router.post('/edit', async (req: Request, res: Response) => {
  ...
  const t = await sequelize.transaction();
  try {
    const specialData = await SpecialImage.findOne({ where: { id: Number(specialId) } });
    const waifusSpecialDefault = await SpecialImageRelation.findAll({ where: { special_image_id: Number(specialId) } });
    console.log(waifusSpecialDefault);
    if (specialData) {
      if (Number(franchiseId) !== specialData.franchise_id) {
        await specialData.update({ franchise_id: franchiseId }, { transaction: t });
      }
    }
