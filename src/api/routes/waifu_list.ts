...
router.get("/", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (waifus.length > 0)
      return res
        .status(200)
        .send({ message: "Estas tu lista", waifus, page, totalPages });
    else
      return res
        .status(201)
        .send({
          message:
            "Lo siento pero no tienes waifus en tu lista, debes ser mas rapido que los demas",
          waifus,
        });
  } catch (error) {
    console.error(error);
...
router.get("/details", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (data.ilegals > data.indefinides && data.ilegals > data.legals)
      message =
        "Dejame decirte que estas en problemas, te gustan mucho las ilegales cuidado con el #FBI que te tiene bien vigilado, aqui el total de waifus en tu lista: ";
    else if (data.legals > data.indefinides && data.legals > data.ilegals)
      message =
        "Dejame decirte que estas en lo legal, no tengo nada que decir solo que aqui esta la cantidad de waifus en tu lista: ";
    else if (data.indefinides > data.legals && data.indefinides > data.ilegals)
      message =
        "Dejame decirte que no estas en problemas pero no sabía que tus gustos eran hacia las mayores, aqui el numero total de tus waifus: ";
    else if (data.ilegals == data.indefinides && data.ilegals == data.legals)
      message =
        "Tu eres una persona completamente neutral, me agradas mucho, aqui esta la cantidad total de tus waifus: ";
    else if (data.ilegals == data.indefinides || data.ilegals == data.legals)
      message =
        "Tus gustos son un tanto extraño, el #FBI te tiene en su lista aunque solo te vigilan aveces, aqui esta la cantidad de waifus que tienes: ";
    else
      message =
        "Las que mas te mas tienes son legales, exelente eres una persona que va por el camino correcto de la vida, aqui el numero de tus waifus: ";
    ...
    return res
      ...
      .send({
        message,
        legals: data.legals,
        ilegals: data.ilegals,
        indefinides: data.indefinides,
        totals: data.totals,
      });
  } catch (error) {
    console.error(error);
...
router.get("/favorites", async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log(waifus);
    if (waifus.length > 0)
      return res.status(200).json({ waifus, totalPages, actualPage: page });
    return res
      .status(201)
      .send({
        message:
          "Parece que todavia no has agregado ninguna waifu a esta lista, lo puedes hacer por el comando /addfavorite <numero en tu lista> <posición>",
      });
  } catch (error) {
    console.error(error);
...
router.put("/deleted_favorite", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (waifuToDelete.length < 1)
      return res
        .status(200)
        .send({ message: "No tienes una waifu en esa posicion" });
    await WaifuFavoriteLists.destroy({
      where: { id: waifuToDelete[0].id },
      transaction: t,
    });
    ...
    await WaifuFavoriteLists.bulkCreate(newPositionAllWaifus, {
      transaction: t,
    ...
    return res
      .status(200)
      .send({
        message: `Se ha eliminado de la lista de favoritos a ${waifuName}`,
      });
  } catch (error) {
    console.error(error);
    await t.rollback();
...
router.put("/trade_proposition", async (req: Request, res: Response) => {
  ...
  const { myWaifuNumber, otherWaifuNumber, chatId, userId, otherUserId } =
    req.body;
  const t = await sequelize.transaction();
  try {
    ...
    const users = await Users.findAll({
      where: { user_id_tg: { [Op.or]: [userId, otherUserId] } },
    });
    ...
    if (myWaifu.length == 0)
      return res
        .status(201)
        .send({
          message: `Tu no tienes una waifu en la posición ${myWaifuNumber} para poder intercambiar`,
        });
    else if (otherWaifu.length == 0)
      return res
        .status(201)
        .send({
          message: `Con quien quieres intercambiar no tiene una waifu en la posición ${otherWaifuNumber}`,
        });
    ...
    if (!trade)
      return res
        .status(201)
        .send({ message: "No se pudo crear la propuesta de intercambio" });
    console.log(myWaifu[0], otherWaifu[0]);
    return res
      .status(200)
      .send({
        myWaifu: myWaifu[0],
        otherWaifu: otherWaifu[0],
        tradeId: trade.id,
      });
  } catch (error) {
    console.error(error);
...
router.put("/trade_answer", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!trade)
      return res.status(401).send("No puedes intervenir en el intercambio");
    ...
    if (userTgId.toString() == trade[0].user_emiter_tg_id.toString())
      return res
        .status(300)
        .send(
          "No tienes permiso para aceptar el intercambio ya que eres el que lo propuso"
        );
    ...
    if (
      (emiter.favorite && emiter.waifu_quantity < 2) ||
      (receptor.favorite && receptor.waifu_quantity < 2)
    ) {
      ...
      return res
        .status(202)
        .send({
          message: `No se puede realizar el intercambio debido a que ${name} de ${franchise} esta marcada como favorita y no se tiene un clon para poder intercambiarla`,
        });
    } else {
      ...
      const waifuAddEmiter = await searchWaifu(
        receptor.waifu_id,
        emiter.id,
        chatId
      );
      const waifuAddReceptor = await searchWaifu(
        emiter.waifu_id,
        receptor.id,
        chatId
      );
  ...
  try {
    ...
    if (franchiseNumber > 0) {
      if (profile.points < 10)
        return res
          .status(205)
          .send("No posees suficientes puntos para obtener una waifu");
      ...
      if (franchise.length == 0)
        return res.status(205).send("No se encontro la franquicia");
      ...
      if (waifuNumber > 0) {
        ...
        if (waifu.length == 0)
          return res.status(205).send("No se encontro la waifu");
        ...
        const index = await Math.round(
          Math.random() * (0 - (waifus.length - 1)) + waifus.length - 1
        );
      ...
      const waifusQuantities: any = await sequelize.query(
        `SELECT COUNT(*) total FROM waifus`,
        { type: QueryTypes.SELECT }
      );
      const waifuId = Math.round(
        Math.random() * (1 - waifusQuantities[0].total) +
          waifusQuantities[0].total
      );
      ...
      const franchiseFormated = {
        ...
        name:
          franchise.nickname == ""
            ? franchise.name
            : franchise.name + " (" + franchise.nickname + ")",
      };
    ...
    await sequelize.query(
      `UPDATE user_infos SET points = points - ${data.cost} WHERE id = ${profile.id}`,
      { transaction: t }
    );

    const waifuInList = await WaifuLists.findOne({
      where: { user_id: user.id, chat_id: chat.id, waifu_id: data.waifu.id },
    });
    if (waifuInList) {
      await sequelize.query(
        `UPDATE waifu_lists SET quantity = quantity + 1 WHERE id = ${waifuInList.id}`,
        { transaction: t }
      );
...
router.post("/delete_list", async (req: Request, res: Response) => {
  ...
  try {
    if (waifuNumber == 0)
      return res
        .status(205)
        .send("Debes enviar un número indicando el la waifu en tu listado");
    const user = await Users.findOne({ where: { user_id_tg: userId } });
    const chat = await Chats.findOne({ where: { chat_id_tg: chatId } });
    if (!user || !chat)
      return res.status(205).send("No se encontro el usuario o el chat");
    const profile = await Profiles.findOne({
      where: { user_id: user.id, chat_id: chat.id },
    });
    ...
    if (waifu[0].quantity <= 1)
      return res
        .status(205)
        .send(
          `Debes tener a ${waifu[0].name} de ${waifu[0].franchise} más de 1 vez para porder cambiarla por puntos`
        );

    const quantityDelete =
      quantity >= waifu[0].quantity
        ? waifu[0].quantity - 1
        : quantity == 0
        ? 1
        : quantity;
    ...
    await sequelize.query(
      `UPDATE user_infos SET points = points + ${quantityDelete} WHERE id = ${profile.id}`,
      { transaction: t }
    );
    await sequelize.query(
      `UPDATE waifu_lists SET quantity = quantity - ${quantityDelete} WHERE id = ${waifu[0].id}`,
      { transaction: t }
    );
...
async function searchWaifu(waifuId: number, userId: number, chatId: number) {
  const waifu = await WaifuLists.findOne({
    where: { user_id: userId, waifu_id: waifuId, chat_id: chatId },
  });
  return waifu;
}

async function addWaifu(waifuListId: number, t: Transaction) {
  await WaifuLists.increment(
    { quantity: 1 },
    { where: { id: waifuListId }, transaction: t }
  );
  return;
}

async function removeWaifu(waifuListId: number, t: Transaction) {
  await WaifuLists.decrement(
    { quantity: 1 },
    { where: { id: waifuListId }, transaction: t }
  );
  return;
...
async function createWaifu(
  ...
  await WaifuLists.create(
    { user_id: userId, waifu_id: waifuId, chat_id: chatId, quantity: 1 },
    { transaction: t }
  );
  return;
}
...
async function reasingPos(allWaifus: [any]) {
  ...
  for (let index = 0; index < length; index++) {
    await newPositionAllWaifus.push({
      id: allWaifus[index].id,
      position: index + 1,
    });
  }
