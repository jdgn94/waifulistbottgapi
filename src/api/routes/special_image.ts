...
const SpecialImage = db.SpecialImageModel;
const SpecialImageRelation = db.SpecialImageRelationModel;
const WaifuList = db.WaifuListModel;
const UserSpecialList = db.UserSpecialListModel;
...
router.get("/", async (req: Request, res: Response) => {
  ...
  const specialsList = await sequelize.query(
    ...
    GROUP BY si.id
    LIMIT 20 OFFSET ${20 * (Number(page) - 1)}
  `,
    { type: QueryTypes.SELECT }
  );

  const specials: any = await sequelize.query(
    `
    SELECT
      COUNT(si.id) total
    FROM
...
router.get("/list", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!user || !chat) throw "Usuario o chat no encontrado";
    ...
    const listId = await Promise.all(await list.map((item: any) => item.id));
    ...
    const specialList = await Promise.all(
      await list.map(async (item: any) => {
        const waifusTemp = await waifus.filter(
          (waifu: any) => waifu.id == item.id
        );
        const waifusTempName = await Promise.all(
          waifusTemp.map((waifu: any) => waifu.name)
        );
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  const waifus = await SpecialImageRelation.findAll({
    where: { special_image_id: id },
  });
...
router.post("/create", async (req: Request, res: Response) => {
  ...
  try {
    ...
    let insert: any = [];
    await waifuIds.forEach(
      async (id: number) =>
        await insert.push({ special_image_id: special.id, waifu_id: id })
    );
...
router.post("/edit", async (req: Request, res: Response) => {
  ...
  const t = await sequelize.transaction();
  try {
    const specialData = await SpecialImage.findOne({
      where: { id: Number(specialId) },
    });
    const waifusSpecialDefault = await SpecialImageRelation.findAll({
      where: { special_image_id: Number(specialId) },
    });
    console.log(waifusSpecialDefault);
    if (specialData) {
      if (Number(franchiseId) !== specialData.franchise_id) {
        await specialData.update(
          { franchise_id: franchiseId },
          { transaction: t }
        );
      }
    ...
    for (let index = 0; index < waifusSpecialDefault.length; index++) {
      waifusSpecialDefaultIds.push(waifusSpecialDefault[index].id);
      const position = await waifuIds.indexOf(waifusSpecialDefault[index].id);
      if (position === -1) waifusDeleted.push(waifusSpecialDefault[index].id);
    }

    let newWaifus = [];
    for (let index = 0; index < waifuIds.length; index++) {
      const exist = waifusSpecialDefaultIds.indexOf(waifuIds[index]);
      if (exist === -1) newWaifus.push(waifuIds[index]);
    }

    if (waifusDeleted.length > 0) {
      SpecialImageRelation.destroy({
        where: { id: waifusDeleted },
        transaction: t,
      });
    }
    ...
    if (newWaifus.length > 0) {
      let insert: any = [];
      await newWaifus.forEach(
        async (id) =>
          await insert.push({ special_image_id: specialId, waifu_id: id })
      );
      await SpecialImageRelation.bulkCreate(insert, { transaction: t });
    }
...
router.post("/add_special", async (req: Request, res: Response) => {
  const { userId, chatId, waifuId } = req.body;
  const t = await sequelize.transaction();
  let addList = false;

  try {
    const waifuInSpecial = await SpecialImageRelation.findAll({
    ...
    for (let i = 0; i < waifuInSpecial.length; i++) {
      ...
      const specialAllWaifus = await SpecialImageRelation.findAll({
        where: { special_image_id: specialId },
      });
      ...
      const waifusUser = await WaifuList.findAll({
        where: {
          waifu_id: specialAllWaifusIds,
          user_id: userId,
          chat_id: chatId,
        },
      });
      ...
      if (specialAllWaifusIds.length === waifusUser.length) {
        await UserSpecialList.create(
          { user_id: userId, chat_id: chatId, special_images_id: specialId },
          { transaction: t }
        );
        addList = true;
      }
    ...
    if (addList)
      return res
        .status(200)
        .send("se ha agregado una nueva imagen a tu listado especial");
